...
function run () {
	...
	for (var skinName in skins) {
		...
		for (var i = skinLayersCount - 1; i >= 0; i--) {
			...
			// Save image.
			// 【修改点】：增加 && layer.kind != LayerKind.TEXT
            // 意思是：如果开启了写图片，并且当前图层不是文本，才执行保存图片
			if (writePngs && layer.kind != LayerKind.TEXT) {
...
if (layer.kind == LayerKind.TEXT) {
    try {
        ...
        jsonDataItem += ',"isText":true';
        jsonDataItem += ',"content":"' + escapeForJson(ti.contents) + '"'; // 内容
        // 提取内容
        var txtContent = escapeForJson(ti.contents);
        
        // 提取字号 (PS单位复杂，这里强制转像素值，Unity端可能需要根据DPI换算)
        var txtSize = 24;
        ...
        // 拼接到 JSON
        jsonDataItem += ',"isText":true'; 
        jsonDataItem += ',"content":"' + txtContent + '"';
        jsonDataItem += ',"fontSize":' + txtSize;
        jsonDataItem += ',"fontColor":"#' + txtColor + '"';
        
        // 如果是文本，你甚至可以在这里决定是否要 writePngs = false (针对当前图层)
        // 但为了兼容特效，通常建议图片和数据都保留
        
    } catch(e) {
        // 如果读取文本属性失败，就当普通图片处理
        jsonDataItem += ',"isText":false,"textError":"' + escapeForJson(e.message) + '"';
    }
} else {
    jsonDataItem += ',"isText":false';
}

// 3. 闭合 JSON 对象
jsonDataItem += '}';

// 4. 将拼接好的字符串赋给 json 变量
json += jsonDataItem;
// ============ 【修改结束】 ============
...
function collectLayers (layer, collect) {
	for (var i = 0, n = layer.layers.length; i < n; i++) {
		...
		else if (child.kind == LayerKind.NORMAL || child.kind == LayerKind.TEXT) {
			collect.push(child);
			child.visible = false;
		}
	}
...
// =========================================================
// 新增辅助函数：将 PS 的 RGB 颜色转为 Hex 字符串 (#RRGGBB)
// =========================================================
